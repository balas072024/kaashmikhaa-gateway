...
USERS = {
    "bala":    {"id":"bala",    "name":"Bala",     "password":"arivu2024bala",  "emoji":"👨","color":"#7c6bff","role":"admin"},
    "wife":    {"id":"wife",    "name":"Wife",     "password":"arivu2024wife",  "emoji":"👩","color":"#00c9a7","role":"member"},
    "child":   {"id":"child",   "name":"Child",    "password":"arivu2024child", "emoji":"👦","color":"#f59e0b","role":"member"},
    "parent1": {"id":"parent1", "name":"Parent 1", "password":"arivu2024p1",   "emoji":"👴","color":"#ff6b6b","role":"member"},
    "parent2": {"id":"parent2", "name":"Parent 2", "password":"arivu2024p2",   "emoji":"👵","color":"#3dd68c","role":"member"},
}
...
def get_host():
    return request.headers.get("X-Forwarded-Host") or request.headers.get("Host") or "arivumaiyam.com"
...
def private_login():
    ...
    if request.method == "POST":
        if request.form.get("password","").strip() == PRIVATE_PASSWORD:
            ...
            host = get_host()
            scheme = get_scheme()
            resp = make_response(redirect(f"{scheme}://{host}{next_url}", 302))
            resp.set_cookie(COOKIE_NAME, token, max_age=30*24*3600,
                           secure=True, httponly=True, samesite="None", domain=COOKIE_DOMAIN)
            return resp
...
def clear_auth():
    ...
    for domain in [COOKIE_DOMAIN, "arivumaiyam.com", "watch.arivumaiyam.com",
                   "family.arivumaiyam.com", "neuralbrain.arivumaiyam.com"]:
...
@app.route("/health")
def health():
    return jsonify({"status":"ok","service":"kaashmikhaa-gateway"})
...
def api_login():
    ...
    token = jwt.encode({"uid":user["id"],"name":user["name"],"role":user["role"],
                        "exp":int(time.time())+7*24*3600}, JWT_SECRET, algorithm="HS256")
    return jsonify({"token":token,"user":{"id":user["id"],"name":user["name"],
                   "emoji":user["emoji"],"color":user["color"],"role":user["role"]}})

@app.route("/", defaults={"path":""})
@app.route("/<path:path>")
def catch_all(path=""):
    host = get_host().lower()
    if path in ("private-login","clear-auth","health") or path.startswith("api/auth/"):
        return app.view_functions.get(path.replace("/","_").replace("-","_"), lambda:("",404))()
    for subdomain, port in PORT_MAP.items():
        sub_clean = subdomain.rstrip(".")
        if host.startswith(subdomain) or host.startswith(sub_clean+"."):
            ...
            if subdomain == "opsshiftpro." and not path.startswith("api/"):
                html = Path("C:/Antigravity/opsshiftpro-mobile/dashboard.html")
                if html.exists():
                    return send_from_directory(str(html.parent), html.name)
            if subdomain == "vault.":
                dist = Path("C:/Antigravity/vault-browser/dist")
                f = dist / path
                if dist.exists():
            ...
            if subdomain == "opswatch.":
                dist = Path("C:/Antigravity/opswatch-unified/frontend/dist")
                f = dist / path
                if dist.exists():
                    if f.exists() and f.is_file():
                        return send_from_directory(str(f.parent), f.name)
    ...
    if tmpl.exists():
        return send_from_directory(str(tmpl.parent), tmpl.name)
    return jsonify({"company":"Kaashmikhaa Technologies","domain":"arivumaiyam.com"})
